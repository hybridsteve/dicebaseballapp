...
// HACK: Gabo edit language enum to see what language is being used
...
public class UIText : System.Object 
{
	...
	// HACK: Gabo edit. Variable for language
	public UITextLanguage Language = UITextLanguage.English;
	...
	{
		_manager = manager;
		//HACK: Changing line to test arabic. 256 is original.
		_fontDetails = new UIFontCharInfo[70000];
			...
			foreach( string word in words )
        	{
				...
				foreach( string word1 in wordsSplit )
					...
					{	
						...
						//Debug.Log("Character ID number: " + idNum );
	...
	private string wrapText( string text, float scale )
	{
		...
		switch( wrapMode )
		{
			...
			{
				// No-op
				// HACK: Gabo edit, adding if elses for languages.
				if( Language == UITextLanguage.Arabic || Language == UITextLanguage.Hebrew )
				{
					//newText = PersianRenderer.RenderStrSingleLine( text );
				}
				else
				{
					newText = text;
			...
			case UITextLineWrapMode.MinimumLength:
			{
				...
				// HACK: Gabo edit. Adding if else statements to check for language
				// TODO: may want to put the code in a method for reusability if it works
				...
				// HACK: Added method for dealing with arabic
				// TODO: May want to take out the for loop and the if statement from above and instead of an else
				// we can use an if for the bottom part that uses the lines, stringbuilder etc.
				else if( Language == UITextLanguage.Arabic )
				{
					spaceLeft = (scaledWrapWidth);
					Debug.Log( "Doing Arabic" );
					Debug.Log( lineWrapWidth );
					for( var i = 0; i < length; i++ )
					{
						var size = wordWidth( words[i], scale );
						Debug.Log( size );
						if( size + spaceWidth > spaceLeft ) 
						{
							...
							// Reset space left on line
							// Gabo edit, Took out the - size part from the line below. When the persian renderer was added to this
							// class instead of being fed the text already converted it was breaking.
							spaceLeft = (scaledWrapWidth);
						} 
						else 
						{
							// Insert word
							// Gabo edit: Since it's arabic, adding the space BEFORE the word.
							newText += " " + words[i];
							spaceLeft = spaceLeft - ( size + spaceWidth );
						}
					...
					{
						//sb.AppendLine( PersianRenderer.RenderStrSingleLine( lines[i], PersianRenderer.NumberMode.English ) );
						//sb.AppendLine( lines[i] );
					}
					newText = sb.ToString();
					
				}
				else if( Language == UITextLanguage.Hebrew )
				{
					Debug.Log( "Doing Hebrew" );
					for( var i = 0; i < length; i++ )
					{
						var size = wordWidth( words[i], scale );
						if( size + spaceWidth > spaceLeft ) 
