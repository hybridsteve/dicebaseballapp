...
{
	// HACK: Gabo edit. Adding a variable for the text wrapper so it can be reused.
	public UISprite TextWrapper;
	...
	{	
		// Need to change back to vertical so it works, if not all the sprites will be layed out on top of each other because of
		// the way it works in layoutChildren.
		if( layoutType == UIAbstractContainer.UILayoutType.AbsoluteLayout )
		{
			layoutType = UIAbstractContainer.UILayoutType.Vertical;
		}
		
		base.addChild( children );
		
		//gabo - setting the layout to not scroll if the contents are smaller than the layout. Right now applied only to vertical
		if( _contentHeight < height )
		{	
			layoutType = UIAbstractContainer.UILayoutType.AbsoluteLayout;
		...
		else if( _contentHeight > height && layoutType == UIAbstractContainer.UILayoutType.AbsoluteLayout )
		{
			layoutType = UIAbstractContainer.UILayoutType.Vertical;
	...
	// HACK: Gabo edit, always scroll to top when transform changed.
