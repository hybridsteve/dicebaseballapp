...
public abstract class UIAbstractTouchableContainer : UIAbstractContainer, ITouchable, IComparable
{
	...
	//hack: skw scrollbar, gabo edit: changed the _scrollBar from private to protected
	protected UIVerticalPanel _scrollBar = null;
	private UISprite _scrollBarInner = null;
	...
	protected override void layoutChildren()
	{
		...
		//skw - good place to position scrollbar, if it exists?
		//gabo - Scrollbar goes crazy if there is less content than the container size so changing some things.
		if (_scrollBar != null && !_scrollBar.hidden )
		{
			//set position
			var newScrollPosition = _scrollPosition / _contentHeight;
			_scrollBar.positionFromTopLeft(-newScrollPosition, 1f);
			
			//set height (ideally we only do this when content changes?)
			var newScrollHeight = ( height ) * ( height / _contentHeight );
			//_scrollBarInner.setSize( 16, newScrollHeight - 32 );
			_scrollBarInner.setSize( 7, newScrollHeight - 18 ); //todo this is dependend on scrollbar graphic dimensions
		}
	}
