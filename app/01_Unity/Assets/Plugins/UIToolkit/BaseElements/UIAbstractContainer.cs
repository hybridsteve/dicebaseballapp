...
public abstract class UIAbstractContainer : UIObject, IPositionable
{
	...
	protected List<UISprite> _uiChildren = new List<UISprite>();
	protected List<UITextInstance> _uiTextChildren = new List<UITextInstance>();
	...
	public float contentWidth { get { return _contentWidth; } }
	public float contentHeight { get { return _contentHeight; } }
	...
	/// <summary>
	/// Gabo Edit.
	/// Add the sprite(s) to the _uiChildren array.
	/// </summary>
	public void addUIChild( params UISprite[] children )
	{
		foreach( var child in children )
		{
			_uiChildren.Add( child );
			//Debug.Log("Adding Sprite Children");
		}
	}
	// HACK: Gabo
	/// <summary>
	/// Gabo Edit.
	/// Add the UITextInstance(s) to the _uiChildren array.
	/// </summary>
	public void addUIChild( params UITextInstance[] children )
	{
		foreach( var child in children )
		{
			_uiTextChildren.Add( child );
			//Debug.Log("Adding TextChildren");
		}
	}
	// HACK: Steve
	/// <summary>
	/// Steve Edit
	/// Animate the alpha to all the UISprites that
	/// are children of this container.
	/// </summary>
	public void alphaTo(float target, float time, UIEaseType ease)
	{
		foreach(var item in _children) 
		{
			item.alphaTo(time, target, ease);
		}
		
		foreach( var item in _uiChildren )
		{
			item.alphaTo(time, target, ease);
		}
	}
